...
Mappings are the first processing step in the <<site-engine#, site engine>> — they run before page rendering starts. You may define mappings for both <<../request-response#, HTTP functions>> and <<../filters#, filters>>. The use cases are vast.
...
<1> *controller* — path to an <<../request-response#, HTTP function>> implementation.
