...
Mappings are the first custom processing step in the <<../sites#, site service>>. Mappings execute before the component rendering. You may define mappings for both <<../http/request-response#, HTTP functions>> and <<../http/filters#, filters>>. The use cases are vast.
...
NOTE: <<../apis#, Universal APIs>> mounted as service points (`+/_/<app>:<api>/+`) cannot be remapped through site mappings — they are routed before the site engine processes mappings. If you need custom behavior on a service-point URL, implement a custom Universal API instead.
