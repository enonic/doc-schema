...
NOTE: You may wire a a filter into the Site engine's execution pipeline via a <<../sites/mappings#, Site mapping>>.
...
Similar to an <<http#, HTTP function>>, standalone filters are triggered based on an export.
