...
Mappings are defined in the <<../sites#, site descriptor>> (`cms/site.yaml`). Examples of use cases are:

* Map a controller to a specific URL pattern i.e. `/mypath`
* Map a controller to a specific set of content through content matches, i.e. only apply to the `portal:fragment` content type
...
<6> *service* specifies a system service handled by the filter or controller (e.g. the `+.../_/image/...+` endpoint). See <<Service mappings>>.
...
NOTE: <<../framework/universal-api#, Universal APIs>> mounted as service points (`+/_/<app>:<api>/+`) cannot be remapped through site mappings — they are routed before the site engine processes mappings. If you need custom behavior on a service-point URL, implement a custom Universal API instead.
