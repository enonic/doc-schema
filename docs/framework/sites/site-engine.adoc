= Site engine and request pipeline
toc: right
:imagesdir: images
...
The site pipeline is executed as a subset of the <<../runtime/web-engine#common_request_pipeline,common pipeline>>:
...
Mappings:: Uses path or content based mappings from site descriptor to trigger filters and controllers
...
The following Universal API's are always mounted on the site's underscore endpoint.
