...
To address a client from outside the `sseEvent` handler, do not store `event.clientId` in module-scope state — controller code runs multi-threaded/multi-engine and ad-hoc shared collections of ids are not safe. Use one of the following instead:
