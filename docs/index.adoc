...
== Introduction
Enonic XP is a highly modular and customizable platform. Build and deploy <<cms#,content schemas>>, custom APIs, functions, admin extensions, id providers, webapps, and even sites.

The <<edk#, Enonic Development Kit>> (EDK) allow developers to quickly start building.
...
Some call it plugins, other call it modules - all we know is that Enonic apps sounds better. Enonic apps are standalone software packages that can be deployed to Enonic XP to extend its functionality. Content Studio and the Headless API are examples of such applications. They are built using the Enonic Development Kit (EDK).

The apps are powered by the <<runtime#, runtime>>, which means multiple applications can run within a single instance. Once an application has started, it has access to all platform services: TODO link to system apis.

== Installation

There are four ways to get your code running on Enonic XP:


Via API:: Use Enonic CLI to push your application to the Management API. The applications will be available for all nodes in an XP cluster. This approach is commonly used in a CI/CD setup.
