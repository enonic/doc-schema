...
* *Events* fire across the cluster. A listener registered on any node receives events published from any other node. See <<../background/events#, Events>>.
...
* *HTTP routing* — four <<../http#, services>> (`/api`, `/admin`, `/site`, `/webapp`) handle incoming requests; you provide the implementations.
* *Identity & access management* — pluggable <<id-providers#, ID providers>>, role-based permissions, audit logging.
* *Background work* — <<../background/tasks#, tasks>>, <<../background/scheduler#, scheduled jobs>>, and <<../background/events#, cluster-wide events>>.
