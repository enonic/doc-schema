...
We default to TypeScript in our code examples and starters. This section describes how to configure and use our official TypeScript types - all aligned with our standard <<../libraries#, JS APIs>>.
...
Along with library types it is *highly recommended* to install definitions for global objects (`app`, `__`, `log`) and functions (`resolve`, `require`):

```bash
npm i --save-dev @enonic-types/global
```

Since `global` types are not placed under the `node_modules/@types`, it is necessary to include them manually:
