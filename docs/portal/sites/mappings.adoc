...
Mappings are the first processing step in the <<site-engine#, site engine>> — they run before page rendering starts. You may define mappings for both <<../../http/request-response#, HTTP functions>> and <<../../http/filters#, filters>>. The use cases are vast.
...
<1> *controller* — path to an <<../../http/request-response#, HTTP function>> implementation.
...
<5> *filter* — path to an <<../../http/filters#, HTTP filter>> implementation.
