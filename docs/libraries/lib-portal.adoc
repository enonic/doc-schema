...
Generates a URL pointing to a <<../framework/universal-api#, Universal API>>. Picks the right URL form based on the calling context — a service-point under the active engine when called from a site, webapp, or admin tool, and `/api/` otherwise.
